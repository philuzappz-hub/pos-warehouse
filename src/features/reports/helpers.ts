...
export function normalizePaymentMethod(value: any) {
  ...
  if (
    v === "momo" ||
    v.includes("momo") ||
    v.includes("mobile money") ||
    v.includes("mobile-money")
  ) {
    return "momo";
  }

  if (v === "card" || v.includes("card") || v.includes("pos")) {
    return "card";
  }
...
export function isValidDateRangeSilent(startDate: string, endDate: string) {
